...
def sampling_func(beta):
    """
    Does sampling using Inverse transform sampling and returns a single sample from custom dist
    """
    u = np.random.uniform()
    return np.log(1 + u * (np.exp(beta) - 1))/beta
...
class RBMSBM(object):
    ...
    def rbm_sample(self, y=None, z=None, chain_length=10, num_samples=1, start_with_z=True):
        ...
        """
        if y is None:
            y = np.random.random(size=(num_samples, self.M)).astype(float)
        ...
        for _ in range(chain_length):
            if start_with_z:
                ...
                z = np.zeros((num_samples, self.K))
                z[np.arange(num_samples), idx] = 1
                y = np.zeros((num_samples,self.M))  # num_samples x M
                for n in range(num_samples):
                    for m in range(self.M):
                        y[n,m] = sampling_func(beta=np.dot(self.W[m,:],z[n,:])+self.b[m]+1e-6)
            else:
                y = np.zeros((num_samples,self.M)) # num_samples x M
                for n in range(num_samples):
                    for m in range(self.M):
    ...
    def variational_e_step(self, edges, A, Y, indices=None, lamda=0.5):
        ...
        for idx in indices:
            a_rep = np.asarray(A[idx, :].reshape((-1, 1)).repeat(self.K, axis=1))
            a_rep_comp = 1 - a_rep
            ...
            temp4 = (a_rep_comp * (q_beta_prod_t - q_sum_prod_t)).sum(axis=0) - q_beta_prod_t[idx, :] + \
                    q_sum_prod_t[idx, :]
            self.q[idx, :] = temp1 + temp2 + temp3 + temp4 + np.matmul(Y[idx, :], self.W) + self.c[:, 0]
    ...
    def variational_m_step(self, Y, num_samples=1, chain_length=10, lr=1e-2, momentum=0.0, use_persistence=False):
        ...
        if not use_persistence or self.y_samples is None:
            self.y_samples = np.asarray(Y[np.random.choice(self.N, size=num_samples), :])
